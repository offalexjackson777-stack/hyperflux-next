...
int main()
{
    ...
    }
    {
        ...
        if(!startup_changed.wait_for(lock,
    ...
    if(!mouse_enqueued || !keyboard_enqueued
        ...
        || keyboard_enqueued.value() != EnqueueDisposition::Accepted || !worker->start())
    {
        return failure(__LINE__);
    }
    ...
    if(worker->enqueue_stable(sdk::LightingIntent::Static,
           {
    ...
    }
    {
        ...
        if(!bounded_changed.wait_for(lock,
               std::chrono::seconds(2),
               [&]
               {
    ...
    if(bounded_worker->enqueue_stable(
           sdk::LightingIntent::Static, {queued("receiver-1/mouse/child.test.mouse", 13, 51)})
        != EnqueueDisposition::Accepted)
    {
        bounded_worker->stop();
        return failure(__LINE__);
